...
export class menuClass {
  ...
  addHandler() {
    if ((!this.offCanvas && mq.max(this.mediaQ)) || Modernizr.touchevents) {
      this.body.addEventListener("click", (event) => this.bodyOnCLick(event));
    }
  ...
  offCanvasBack() {
    ...
    if (selectedMenu.length > 0) {
      [...selectedMenu].forEach((item, i) => {
        ...
        const parentMenu = item.closest(`.${this.SUB_MENU}.${this.ACTIVE}`);
  ...
  arrowOnClick(event) {
    ...
    const closestSubmenu = closestMenuItem.querySelector(`.${this.SUB_MENU}`);
      ...
      case "SUBMENU_IS_CLOSE":
        ...
        switch (menuType) {
          case "IS_OFFCANVAS":
            ...
            if (mq.max(this.mediaQ)) closestSubmenu.style.top = `${gap}px`;
            break;
