...
class mouseManagerClass {
    constructor() {
        ...
        (this.ticking = false), (this.target = null);
        (this.stackId = -1), (this.lastScrolledTop = 0);
    }
    ...
    execute(_properties) {
        if (this.data.hasOwnProperty(_properties)) {
            for (
                let index = 0;
                index < this.data[_properties].length;
                index++
            ) {
    ...
    remove(_properties, _id) {
        this.data[_properties] = this.data[_properties].filter(
            (obj) => obj.id != _id
        );
    }
