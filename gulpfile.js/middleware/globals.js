...
const ssgCategoryName = (id) => {
    return propValidate(
        [id, 'name', templateFunction.ssgLang],
        config.categoryLocales
    )
...
const ssgCategoryPermalink = (id) => {
    return id && propValidate([id, 'indexUnivoqueId'], config.categoryLocales)
        ? templateFunction.ssgPermalink(
              config.categoryLocales[id].indexUnivoqueId
          )
        : null;
};
...
const ssgPageTitle = (univoqueId) => {
    return propValidate(
        [univoqueId, templateFunction.ssgLang],
        store.pageTitleMapData
    )
 ...
 */
const ssgBodyClass = () => {
    const univoqueIdnoSlash = templateFunction.ssgUnivoqueId.replace(
        /\//g,
        '-'
    );
...
const ssgDate = (date) => {
    ...
    return propValidate(
        [templateFunction.ssgLang, 'code'],
        templateFunction.ssgLocales
    )
        ? newDate.toLocaleDateString(
              templateFunction.ssgLocales[templateFunction.ssgLang].code,
              options
          )
        : date;
};
...
const getAlternateLangCode = () => {
    ...
    return Object.entries(locale)
        .filter((item) => {
            ...
            const isDraft = propValidate(
                [templateFunction.ssgUnivoqueId, lang],
                store.permalinkMapData
            )
                ? templateFunction.ssgIsDraft(templateFunction.ssgUnivoqueId)
                : true;

            return lang !== templateFunction.ssgLang && !isDraft;
        })
        .map((item) => {
            const [lang, langData] = item;
            const langCode = 'code' in langData ? langData.code : '';
            return langCode;
        });
};
